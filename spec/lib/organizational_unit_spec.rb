...
describe OrganizationalUnit do
  ...
  context 'when affiliations param responds to #affilations' do
    ...
    it 'returns the org unit' do
      expect(Settings).to receive_message_chain(:organizational_units, :person_emails).and_return :'/Employees' => ['employee']
  ...
  context 'when affiliations match one org unit' do
    ...
    it 'returns the org unit' do
      expect(Settings).to receive_message_chain(:organizational_units, :person_emails).and_return :'/Employees' => %w{employee}
      expect(subject).to eql '/Employees'
    end
  end
  ...
  context 'when affiliations match multiple org units' do
    ...
    it 'returns the org unit from the first match' do
      expect(Settings).to receive_message_chain(:organizational_units, :person_emails).and_return :'/Employees' => %w{faculty employee}, :'/Alumni' => %w{alumnus}
      expect(subject).to eql '/Employees'
    end
  end
