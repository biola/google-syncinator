...
describe Emails::NotifyOfInactivity do
  ...
  it 'sends an email' do
    ...
    expect { Emails::NotifyOfInactivity.new(deprovision_schedule).send! }.to change { Mail::TestMailer.deliveries.length }.from(0).to 1
  end

  it 'sends to the right recipient' do
    expect(TrogdirPerson).to receive(:new).and_return(double(first_or_preferred_name: 'Bob'))
    Emails::NotifyOfInactivity.new(deprovision_schedule).send!
