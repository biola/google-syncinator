...
describe Workers::Deprovisioning::Suspend do
  ...
  context 'when deprovision schedule canceled' do
    let!(:schedule) { email.deprovision_schedules.create action: :suspend, scheduled_for: 1.minute.ago, canceled: true }
  ...
  context 'when deprovision schedule not canceled' do
    let!(:schedule) { email.deprovision_schedules.create action: :suspend, scheduled_for: 1.minute.ago }
