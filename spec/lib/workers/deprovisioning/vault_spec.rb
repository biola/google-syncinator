require 'spec_helper'
# TODO: Finish me. Or rather start me..
describe Workers::Deprovisioning::Vault, type: :unit do
  let(:uuid) { '00000000-0000-0000-0000-000000000000' }
  let(:address) { 'bob.dole@biola.edu' }
  let!(:email) { create :person_email, uuid: uuid, address: address }
  let(:reason) { nil }
  let!(:schedule) { email.deprovision_schedules.create action: :vault, scheduled_for: 1.minute.ago, canceled: canceled, reason: reason }

  context 'when deprovision schedule canceled' do
    let(:canceled) { true }

    it 'does not vault Google account' do
      expect_any_instance_of(GoogleAccount).to_not receive(:vault!)
